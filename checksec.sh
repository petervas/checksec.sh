...
# Name    : checksec.sh
# Version : 4.0.0
# Author  : Peter Vaskovic
# Date    : January 2025
# Download: https://github.com/petervas/checksec.sh
#
# --- Modified Version ---
# Name    : checksec
# Version : 2.7.1
# Author  : Brian Davis
# Date    : June 2024
# Download: https://github.com/slimm609/checksec
#
# --- Modified Version ---
# Name    : checksec.sh
...
for command in cat awk sed sysctl uname grep stat file find sort head ps readlink id xargs ldconfig tr readelf; do
...
search_libc() {
  if [[ -z ${FS_libc} ]]; then
    # if a specific file or search path is given, use it
    if [[ -n "${LIBC_FILE}" ]]; then
      if [[ -f "${LIBC_FILE}" ]]; then
        FS_libc=${LIBC_FILE}
    ...
    # otherwise use ldconfig to get the systems libc location
    ...
    if [[ -z ${FS_libc} ]]; then
      # if a search path was specified, look for libc.so.6, libc.so.7 or libc.so in LIBC_SEARCH_PATH in prioritized order
      if [[ -n "${LIBC_SEARCH_PATH}" ]]; then
        FS_libc=$(find "${LIBC_SEARCH_PATH}" \( -type f -o -type l \) \( -name "libc.so.6" -o -name "libc.so.7" -o -name "libc.so" \) 2>/dev/null | { grep "libc.so.6" -m 1 || grep "libc.so.7" -m 1 || grep "libc.so" -m 1; })
      # if ldconfig failed, then as a last resort search for libc in "/lib/", "/lib64/", "/lib32/" and "/"
      else
        FS_libc=$(find /lib/ /lib64/ /lib32/ / \( -type f -o -type l \) \( -name "libc.so.6" -o -name "libc.so.7" -o -name "libc.so" \) 2> /dev/null | { grep "libc.so.6" -m 1 || grep "libc.so.7" -m 1 || grep "libc.so" -m 1; })
      fi
    fi
...
chk_fortify_file() {
  ...
  use_dynamic=
  if [[ $(${readelf} -s "${CHK_FORTIFY_FILE}" 2> /dev/null) =~ "Dynamic symbol information is not available" ]]; then
  ...
  while IFS='' read -r line; do FS_functions+=("$line"); done < <(${readelf} -s "${use_dynamic}" "${CHK_FORTIFY_FILE}" 2> /dev/null | awk '{ print $8 }' | sed 's/_*//' | sed -e 's/@.*//' | sort -u)
...
chk_fortify_proc() {
  ...
  # fallback on dynamic section to retrieve symbols when symbol table is unavailable
  use_dynamic=
  if [[ $(${readelf} -s "${CHK_FORTIFY_PROC}/exe" 2> /dev/null) =~ "Dynamic symbol information is not available" ]]; then
    use_dynamic="--use-dynamic"
  fi
    ...
    FS_functions=()
    while IFS='' read -r line; do FS_chk_func_libc+=("$line"); done < <(${readelf} -s "${FS_libc}" 2> /dev/null | grep _chk@@ | awk '{ print $8 }' | cut -c 3- | sed -e 's/_chk@.*//' | sort -u)
    while IFS='' read -r line; do FS_functions+=("$line"); done < <(${readelf} -s "${use_dynamic}" "${CHK_FORTIFY_PROC}/exe" 2> /dev/null | awk '{ print $8 }' | sed 's/_*//' | sed -e 's/@.*//' | sort -u)
...
chk_proc() {
  ...
  cd /proc || exit
  # assume a process name was given (pgrep can only use up to 15 characters to identify processes)
  IFS=" " read -r -a fpids <<< "$(pgrep -d ' ' "${CHK_PROC:0:15}")"
...
debug_report() {
  ...
  for command in cat awk sed sysctl uname grep stat file find sort head ps readlink id xargs ldconfig tr readelf eu-readelf; do
...
filecheck() {
  ...
  if file -b "${1}" 2> /dev/null | grep -q "static-pie linked\|statically linked"; then
    echo_message '\033[32mN/A            \033[m   ' 'N/A,' ' canary="n/a"' '"canary":"n/a",'
  elif ${readelf} -s "${use_dynamic}" "${1}" 2> /dev/null | grep " UND " | grep -Eq '__stack_chk_fail|__stack_chk_guard|__intel_security_cookie'; then
  ...
  if ${extended_checks}; then
    ...
    if [ -n "${use_dynamic}" ] || ! readelf -S "${1}" | grep -q '\.symtab'; then
      echo_message '\033[32mN/A (no symtab)   \033[m   ' 'N/A,' ' clangcfi="n/a"' '"clangcfi":"n/a",'
    else
...
proccheck() {
  ...
  # fallback on dynamic section to retrieve symbols when symbol table is unavailable
  use_dynamic=
  if [[ $(${readelf} -s "${1}/exe" 2> /dev/null) =~ "Dynamic symbol information is not available" ]]; then
    use_dynamic="--use-dynamic"
  fi

    # check for stack canary support
  if file -L "${1}/exe" 2> /dev/null | grep -q "static-pie linked\|statically linked"; then
    echo_message '\033[32mN/A                  \033[m   ' 'N/A,' ' canary="n/a"' '"canary":"n/a",'
  elif ${readelf} -s "${use_dynamic}" "${1}/exe" 2> /dev/null | grep " UND " | grep -Eq '__stack_chk_fail|__stack_chk_guard|__intel_security_cookie'; then
    echo_message '\033[32mCanary found         \033[m   ' 'Canary found,' ' canary="yes"' '"canary":"yes",'
  elif [[ "${1}" == "1" ]]; then
    echo_message '\033[33mPermission denied    \033[m   ' 'Permission denied,' ' canary="Permission denied"' '"canary":"Permission denied",'
  elif ! ${readelf} -s "${use_dynamic}" "${1}/exe" 2> /dev/null | grep -q 'Symbol table'; then
  ...
  if ${extended_checks}; then
    ...
    #if $readelf -s "$1" 2>/dev/null | grep -Eq '\.cfi'; then
    if [ -n "${use_dynamic}" ] || ! readelf -S "$1/exe" | grep -q '\.symtab'; then
      echo_message '\033[32mN/A (no symtab)   \033[m   ' 'N/A,' ' clangcfi="n/a"' '"clangcfi":"n/a",'
    else
      read -r cfifunc <<< "$(${readelf} -s "$1/exe" 2> /dev/null | grep '\.cfi' | awk '{ print $8 }')"
      func=${cfifunc/.cfi/}
    ...
    if ${readelf} -s "${use_dynamic}" "$1/exe" 2> /dev/null | grep -Eq '__safestack_init'; then
      echo_message '\033[32mSafeStack found   \033[m   ' 'with SafeStack,' ' safestack="yes"' '"safestack":"yes",'
    else
      echo_message '\033[31mNo SafeStack found\033[m   ' 'without SafeStack,' ' safestack="no"' '"safestack":"no",'
    fi
  ...
  Proc_FS_filechk_func_libc="$(${readelf} -s "${FS_libc}" 2> /dev/null | sed -ne 's/.*__\(.*_chk\)@@.*/\1/p')"
